...
namespace Hands_of_Cards
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int sum = 0;
            string[] hands = Console.ReadLine().Split(new char[] { ' ', ',', ':' }, StringSplitOptions.RemoveEmptyEntries).ToArray();
            string names = "";
            List<string> cards = new List<string>();
            while (hands[0] != "JOKER")
            {
                names = hands[0];
                cards = hands.Skip(1).ToList();
                cards = cards.Distinct().ToList();
                if (!players.ContainsKey(names))
                {
                ...
                hands = Console.ReadLine().Split(new char[] { ' ', ',', ':' }, StringSplitOptions.RemoveEmptyEntries).ToArray();
            }
            int multiplayer = 0;
            int mainValue = 0;
